...
import { Device, UiInteraction, PrevDeviceId } from 'types/connect';

export const isNotConnected = ({ device }: { device?: Device }) => device && device !== null && device.connected !== true;

export const isNotSameDevice = ({ device, prevDeviceId }: { device?: Device, prevDeviceId: PrevDeviceId }) => {
...
// todo typescript
export const isRequestingPin = ({ device, uiInteraction }: { device?: Device, uiInteraction: UiInteraction }) => {
    if (!device || !device.features) {
        return null;
    }
