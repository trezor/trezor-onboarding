...
class ProgressSteps extends React.Component<Props> {
    ...
    getStepsWithDots() {
        ...
        if (isSecurityActive) {
            steps = this.props.steps.reduce((accumulator: Array<Step>, current: Step) => {
    ...
    isStepFinished(steps: Array<Step>, index: number, activeStep: Step) {
        const activeStepIndex = steps.findIndex((s:Step) => s.title === activeStep.title);
